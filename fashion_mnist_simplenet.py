...
import numpy as np
from models.simplenet_mnist import simpleCNN, HartleySpectralSimpleCNN, CosineSpectralSimpleCNN
...
if __name__=='__main__':
    ...
    for i in range(1, args.run_num+1):
        ...
        if args.pool == 'hartley':
            model = HartleySpectralSimpleCNN()
        if args.pool == 'cosine':
            model = CosineSpectralSimpleCNN()
        print('#Parameters: %d' % count_params(model))
        # mkdir a new folder to store the checkpoint and best model
        if not os.path.exists('result'):
            os.makedirs('result')
        fdir = os.path.join('result/fashion_mnist/simple', args.pool)
