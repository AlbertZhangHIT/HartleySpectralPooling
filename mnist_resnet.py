...
parser.add_argument('--pool', default='', type=str, help='pooling method', choices=['regular', 'hartley', 'cosine'])
...
if __name__=='__main__':
    ...
    for i in range(1, args.run_num+1):
        ...
        if args.pool == 'regular':
            ...
            model = spresnet15_mnist(s_pool=args.pool)
        print('#Parameters: %d' % count_params(model))
        # mkdir a new folder to store the checkpoint and best model
        if not os.path.exists('result'):
            os.makedirs('result')
        fdir = os.path.join('result/mnist/resnet', args.pool)
