...
import numpy as np
from models.fashion_mnist import FashionCNN, HartleyPoolFashionCNN, CosinePoolFashionCNN
...
parser.add_argument('--pool', default='max', type=str, help='pooling method', choices=['max', 'hartley', 'cosine'])
...
if __name__=='__main__':
    ...
    for i in range(1, args.run_num+1):
        ...
        if args.pool == 'max':
            model = FashionCNN()
        if args.pool == 'hartley':
            model = HartleyPoolFashionCNN()
        if args.pool == 'cosine':
            model = CosinePoolFashionCNN()
        print('#Parameters: %d' % count_params(model))
        # mkdir a new folder to store the checkpoint and best model
        if not os.path.exists('result'):
            os.makedirs('result')
        fdir = os.path.join('result/fashion_mnist/cnn', args.pool)
        ...
        if args.evaluate:
            ...
            for epoch in range(args.start_epoch, args.epochs):
                ...
                print('Best Prec: {:.3f}%'.format(best_prec[i-1]))
                save_checkpoint({
                    'epoch': epoch + 1,
